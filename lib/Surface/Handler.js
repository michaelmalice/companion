...
class SurfaceHandler extends CoreBase {
	...
	drawPage() {
		if (!this.lockedout) {
			...
			for (let i in data) {
				...
				let pressed = false
				this.system.emit('graphics_is_pushed', this.page, i, (_pressed) => {
					pressed = _pressed
				})
	...
	onDeviceClick(devicepath, key, state) {
		...
		if (!this.lockedout) {
			...
			this.system.emit('bank_pressed', this.currentPage, key + 1, state, this.panel.serialnumber)
	...
	updateBank(page, bank) {
		...
		if (!this.lockedout) {
			...
			if (page == this.currentPage) {
				...
				let pressed = false
				this.system.emit('graphics_is_pushed', page, bank, (_pressed) => {
					pressed = _pressed
				})
