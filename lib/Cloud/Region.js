...
class CloudRegion {
	...
	constructor(cloud, id, data) {
		...
		this.io = cloud.io
		this.log = cloud.log
		this.debug = debug0(`lib/Cloud/${id}`)
			...
			while (this.socket) {
				for await (let _event of this.socket.listener('connect')) {
					...
					if (this.cloud.state.uuid === '') {
						...
						this.log('error', 'Error logging into cloud: Error fetching unique machine id')
						return
					}
					...
					try {
						...
						this.log('error', 'Error logging into cloud: ' + e.message)
					}
				}
				await delay(1000)
