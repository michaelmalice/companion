...
const debug = debug0('lib/Trigger/Plugin/Feedback')
...
class TriggersPluginFeedback extends TriggersPluginBase {
	setup() {
		...
		this.scheduler.system.on('triggers_update_feedback_values', this.update_feedback_values.bind(this))
	...
	update_feedback_values(values, instance_id) {
	...
	_cond_desc(feedback) {
		...
		let instanceConfig
		this.scheduler.system.emit('instance_config_get', feedback.instance_id, (config) => {
			instanceConfig = config
		})
		const instanceLabel = instanceConfig ? instanceConfig.label : 'Missing'
