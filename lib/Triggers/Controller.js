...
class TriggersController extends CoreBase {
	...
	constructor(registry) {
		super(registry, 'triggers', 'lib/Triggers/Controller')
	...
	action(id) {
		...
		this.log('info', `Execute ${event.title}`)
		...
		// 	this.log('info', `Release button ${bank}.${button} via ${event.title}`)
	...
	test_actions(socket, title, actions, relative_delays) {
		this.log('info', `Testing execution for ${title}`)
