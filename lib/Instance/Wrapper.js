...
class SocketEventsHandler {
	...
	async sendAllFeedbackInstances() {
		...
		let banks = {}
		this.registry.system.emit('get_all_banks', (_banks) => {
			banks = _banks
		})
	...
	async feedbackUpdate(feedback, page, bank) {
		...
		let page_banks = {}
		this.registry.system.emit('get_banks_for_page', page, (_res) => {
			page_banks = _res
		})
	...
	async #handleLogMessage(msg) {
		this.registry.system.emit('log', 'instance(' + this.label + ')', msg.level, msg.message)
	}
	/**
	...
	async #handleSendOsc(msg) {
		this.registry.system.emit('osc_send', msg.host, msg.port, msg.path, msg.args)
	}
}
